...
public class DNA
{
    ...
    private readonly int mutationChance = 10;//10% chance to mutate
    ...
    public void Mutate()
    {
        if (Random.Range(0, 100) < mutationChance) {
            ...
            genes[randomGene] = Mathf.Clamp(
                genes[randomGene] + Random.Range(-maxValues[randomGene] / 5, maxValues[randomGene] / 5),
                minValues[randomGene],
                maxValues[randomGene]
            );
        }
    }
